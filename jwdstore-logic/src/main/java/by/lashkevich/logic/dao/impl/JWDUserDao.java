...
public class JWDUserDao implements UserDao {
    ...
    private static final String FIND_BASKET_BY_USER_ID_SQL = "SELECT baskets.user_id AS user_id, baskets.good_id" +
            ...
            " review_user_balance, users.role AS review_user_role FROM baskets LEFT JOIN goods ON baskets.good_id = goods.id" +
    ...
    private static final String ADD_GOOD_IN_BASKET_SQL = "INSERT INTO baskets (good_id, user_id, quantity)" +
            " VALUES (?, ?, ?)";
    private static final String FIND_GOOD_QUANTITY = "SELECT baskets.quantity AS good_quantity FROM baskets" +
            " WHERE user_id = ? AND good_id = ?";
    private static final String REMOVE_GOOD_FROM_BASKET = "DELETE FROM baskets WHERE user_id = ? AND good_id = ?";
    ...
    public Optional<Basket> findBasketByUserId(Long userId) throws DaoException {
        try (PreparedStatement statement = connection.prepareStatement(FIND_BASKET_BY_USER_ID_SQL)) {
            Optional<User> basketOwner = findById(userId);
            if (!basketOwner.isPresent()) {
            ...
            basket.setOwner(basketOwner.get());
    ...
    public boolean clearBasketByUserId(Long userId) throws DaoException {
        try (PreparedStatement preparedStatement = connection.prepareStatement(REMOVE_BASKET_BY_USER_ID_SQL)) {
    ...
    @Override
    public boolean addGoodInBasket(Long goodId, Long userId, int quantity) throws DaoException {
        try (PreparedStatement preparedStatement = connection.prepareStatement(ADD_GOOD_IN_BASKET_SQL)) {
    ...
    @Override
    public boolean removeGoodFromBasket(Long goodId, Long userId, int quantity) {
        return false;
    }
    ...
    private int findGoodQuantity(Long userId, Long goodId) throws SQLException {
        try (PreparedStatement preparedStatement = connection.prepareStatement(FIND_GOOD_QUANTITY)) {
            ...
            resultSet.next();
            return resultSet.getShort(GOOD_QUANTITY);
        }
    }

    private void clearGoodFromBasket(Long userId, Long goodId) throws SQLException {
        try (PreparedStatement preparedStatement = connection.prepareStatement(REMOVE_GOOD_FROM_BASKET)) {
            preparedStatement.setLong(1, userId);
            preparedStatement.setLong(2, goodId);
